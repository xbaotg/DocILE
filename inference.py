...
from model.roberta_model import MyXLMRobertaMLForTokenClassification
...
if __name__ == "__main__":
    ...
    config = [
        ...
        for path in CFG_FILE.MODEL_PATHS
    ]
    ...
    primary_tokenizer = tokenizer[0]

    model = [
        MyXLMRobertaMLForTokenClassification.from_pretrained(path, config=conf).to(device)
        for (path, conf) in zip(CFG_FILE.MODEL_PATHS, config)
    ]
    primary_model = model[0]

    for i in range(len(model)):
        try:
            model[i].config.id2label[model[i].config.label2id["B-variable_symbol"]] = "B-payment_reference"
            model[i].config.id2label[model[i].config.label2id["I-variable_symbol"]] = "I-payment_reference"
            print(f"INFO: model[{i}] with an obsolete label set. Updating the label-set...")
        except Exception:
            print(f"INFO: model[{i}] with up-to-date label set")
    ...
    for document in tqdm(dataset):
        ...
        for page in range(document.page_count):
            ...
            ocr = document.ocr.get_all_words(page, snapped=True, get_page_image=lambda: img)
            ocr = [
            ...
            def get_output(text_tokens, config, model, tokenizer):
                ...
                tokenized_inputs["bboxes"] = (
                    torch.Tensor(np.array(bboxes2, dtype=np.int32)).long().to(model.device)
                )
            ...
            tokenized_inputs, outputs = None, [None for _ in range(len(model))]

            for i in range(len(model)):
                tokenized_inputs, outputs[i] = get_output(text_tokens, config[i], model[i], tokenizer[i])

            # ensemble here
            predictions, scores = CFG_FILE.ensemble_output(outputs)
            num_batch = outputs[0].logits.shape[0]
            ...
            for b_i in range(num_batch):
                if b_i > 0:
                    start_i = primary_model.config.stride + 1
                    end_i = np.where(
                        np.array(tokenized_inputs[b_i].tokens) == primary_tokenizer.sep_token
                    )[0][0]
                else:
                    start_i = 1
                    end_i = -1
                ...
                tokens.extend(
                    primary_tokenizer.convert_ids_to_tokens(tokenized_inputs["input_ids"].tolist()[b_i])[
                        start_i:end_i
                    ]
                )
            ...
            for pred, score in zip(predictions_flattened, scores_flattened):
                pred_classes_full.append(
                    [(primary_model.config.id2label[x], score[x]) for x in np.nonzero(pred)[0]]
                )
