...
from helpers import MyMLDataCollatorForTokenClassification
...
if __name__ == "__main__":
    ...
    # we use this to get len(trainer.get_train_dataloader()) which is used as a parameters for custom Optimizer
    trainer = Trainer(
        model=model,
        args=training_args,
    ...
    t_total = int(len(trainer.get_train_dataloader()) // args.gradient_accumulation_steps * args.num_epochs)
    ...
    trainer = Trainer(
        ...
        data_collator=data_collator,
        compute_metrics=compute_metrics,
        optimizers=CFG_FILE.get_optimizer(model, args.lr, args.weight_decay, args.warmup_steps, t_total),
    )
